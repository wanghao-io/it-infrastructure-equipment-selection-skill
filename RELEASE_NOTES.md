...
v1.4.0 增加一个薄 CLI 入口，用于发现、示例化和运行现有确定性计算器及 Schema 校验；原脚本和计算公式保持兼容，Agent 选型、询价与实时研究不会被伪装成普通计算命令。

v1 契约正式冻结并继续支持，破坏性增强进入带版本路径和 `$id` 的 v2。价格证据 v2 强制单一决策作用域和显式技术门禁，真实项目复盘 v2 为运行测量及预算预测比较增加结构化证据约束。迁移工具默认只输出审计报告，不修改源文件，也不会自动补造 `PASS`、`Verified`、零或当前日期。

私有模板、报价和产品事实采用“公开核心 + 私有适配器 + 受控数据源”的明确边界：不自动扫描私有目录，不允许供应商输入直接声明技术通过或证据等级，公共仓库不保存客户、联系人或报价原件。

## 全品类价格门禁与证据作用域热修

v1.3.1 将价格锚点改为全品类 fail-closed：服务器、固定 SKU 和通用组件都必须先明确通过技术适配并允许进入价格比较。一个供应商的多个报价号不再被误算为多个独立来源，不同 BOM 行、候选族或项目的证据也不能混成同一预算区间。

价格决策入口新增严格契约预检，旧的无版本输入只能通过显式兼容参数使用并产生警告。真实项目复盘同时增加阶段语义门禁：设计阶段不能声称已有结算或运行证据，成交、结算和运行测量必须提供对应的结构化记录。
...
v1.2.2 在 v1.2.1 的前三项完整性修复上进一步封堵边界：预算区间只要下界低于旧预算，就必须经过通用技术适配门禁，不能用高于旧预算的上界绕过；服务器供应商、渠道和报价号必须是有效文本，供应商独立性按大小写与空白归一化；采购成本统一走严格数值校验。

新增的真实 CLI 端到端回归会依次执行服务器询价比较、用同一批合格报价修订现有预算，并校验 HCI N+1 的 CPU、内存、可用存储、IOPS 和网络输出，不再只检查模拟项目标签。
