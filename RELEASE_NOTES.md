...
v1.1.1 是一次以**价格准确性**为重点的修复版本，主要面向服务器、存储、防火墙、HCI、企业级 UPS 等高度可配置设备的项目询价、BOM 预算和采购比价场景。

这一版本重点解决一个实际项目中很容易出现的问题：

> **同一个机箱型号或产品系列，不代表同一个采购配置，也不能直接拿公开起售价、裸机价或历史成交价去估算完整配置的当前采购价格。**

核心规则调整为：

> **技术参数以厂商权威资料为准；价格锚点以配置匹配度、当前可采购性和完整商业范围为准。**
...
v1.1.1 is a pricing-accuracy release focused on enterprise hardware quotation, BOM budgeting and procurement comparison for highly configurable equipment such as servers, storage, firewalls, HCI nodes and enterprise UPS systems.
...
Procurement objects are classified before price-channel selection:
...
No migration is required from v1.1.0. Existing users can update the skill directory and immediately use the new live price-research and evidence-ranking rules.
